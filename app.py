...
def predict():
    try:
        # Grab the user's phone number from the hidden input!
        user_phone = request.form.get("user_phone", "").strip()
        
        location_name = request.form.get("location", "Unknown Location").strip() or "Unknown Location"
        ...
        regional_df = load_regional_profiles()
        profile = get_region_record(regional_df, location_name)

        try:
            probabilities = model.predict_proba(scaled)[0]
            classes = model.classes_
            top_crops = build_top_crop_results(probabilities, classes, profile=profile)
...
def predict_soil():
    try:
        ...
        # 2. Safety check: Did the CSV load at all?
        

        # 3. SMART MATCHING: Check if the CSV location word is *inside* the long user input
        # e.g., Is "bhubaneswar" inside "bhubaneswar municipal corporation"?
        

        # 4. Fallback: If the location is completely unknown (e.g., "New York"), just match the crop!
       

        # 5. Extract the matched fertilizer string
        if fert_model:
            try:
        ...
        return jsonify({
            "result": f"🌱 Fertilizer: {base_fertilizer}",
            "tips": suggestions
        })
